...
On the ISCWSA standard set the exact method returns the **same collision/clear verdict as the validated rule on every well** (Wells 03, 04, 09, 10, 11 collide; 01, 02, 05, 06, 07, 08 clear), while being **less conservative on the margin** — up to $1.47\times$ (Well 03: $\mathrm{SF}$ $0.46\to0.67$; Well 05: $1.20\to1.72$; Well 07: $1.63\to2.18$). Figure 2 contrasts the two across the set. The method is **fast**: tens of milliseconds per well pair, several-fold faster than the pedal rule on the same hardware — well inside the inner loop of batch and automated well planning.
...
Figure 3 shows the same effect *along* a well (offset Well 05): the separation factor falls to its minimum at the closest approach, and the gap between the rule and the exact method widens toward that minimum — it is largest exactly where the go/no-go decision is taken. (This depth view is shown for a single well: per station the rule and the exact metric select *different* closest offsets, so their depth profiles cross away from the governing minimum; the all-wells comparison at the governing minimum is Figure 2.)

![Separation factor versus depth along the reference well (offset Well 05). SF dips to its minimum at the closest approach; the conservatism gap — the exact Mahalanobis factor (blue) to the right of the rule (orange) — is small where the wells are far apart and opens toward the closest approach, where the decision is made. Log SF axis, clipped to the decision-relevant range (far-field SF is off-scale); the shaded band is the $\mathrm{SF}<1$ collision zone.](figures/sf-vs-depth.png){width=70%}
