...
This is exactly the broadphase-plus-narrowphase structure of a mesh / collision-library query (e.g. `fcl`): there, the swept-tube mesh interpolates the surface between stations and the library locates the contact; here that interpolation and contact-finding are performed directly on the analytic ellipsoid. Two consequences follow. First, the method needs **no mesh and no collision library** — the heavyweight `trimesh`/`fcl`/`rtree` stack is unnecessary for a pairwise separation factor. Second, the resolution is **self-derived from the geometry** (the survey's own stations) and the result is **step-free** (an optimiser tolerance), so it scales without a domain-specific tuning parameter. Validated against an exhaustive all-pairs reference sampled at $1$ m, the method agrees to better than $10^{-3}$ in separation factor on every well of the ISCWSA standard set, at $\approx 35$ ms per pair.

For *multi-well scenes* — one planned well against many offsets, with spatial broadphase culling — the same combined-covariance ellipsoid can still be realised as a triangulated mesh and tested with a collision manager; that is where a mesh library earns its keep (many-body broadphase), not in the pairwise factor.
...
**On replacing a collision library.** Because both bodies here are *analytic* (covariance ellipsoids swept along curves) and the metric is a quadratic form, the proximity query has a closed-form kernel; meshing them and calling a general collision library (`fcl` via `trimesh`) is then unnecessary — for the pairwise factor, and, with a spatial-index broadphase over curve bounding volumes, for multi-well scenes too. This generalises to *any* proximity problem whose bodies admit a tractable distance function (ellipsoids, quadrics, convex primitives, signed-distance fields) — a broad class that includes all covariance/uncertainty-ellipsoid problems. It does **not** make a general collision library redundant: arbitrary non-analytic mesh or CAD geometry (bit/BHA bodies, casing, robotics) has no closed-form distance and still requires mesh-based BVH/GJK methods. The claim here is bounded — exact, dependency-free proximity for *analytic* shapes — not a universal replacement.
