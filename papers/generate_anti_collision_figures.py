...
# Generated by the library's consistent Clearance.sf_vs_md() — both the pedal
# (eq 2) and exact (eq 5) profiles from one kernel over the same station pairing,
# so the figure uses the shipped, tested method, not a paper-only re-derivation.
def fig4():
    ...
    for ax, (w, lab) in zip(axes, wells):
        md, ped, mah = MahalanobisClearance(ref, gs[w]).sf_vs_md()
