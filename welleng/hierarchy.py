...
``Organisation``) and entity-relationship docs under ``E-R/master-data/``.
...
between wells of common ancestry.
...
class WellNetwork:
    ...
    def __init__(self) -> None:
        # forest storage: single parent per node, ordered children per parent
    ...
    def add(self, node: _Node) -> _Node:
        ...
        if isinstance(parent, (Well, Wellbore)):
            ...
            self._children.setdefault(parent.id, [])
            if node.id not in self._children[parent.id]:
                self._children[parent.id].append(node.id)
            self._parents[node.id] = parent.id
    ...
    def leaves(self) -> list[Wellbore]:
        ...
        return [
            n for n in self._wellbores()
            if not self._children.get(n.id)
        ]
