...
Two hard requirements (JJ 2026-07-05):
...
Status: SCAFFOLD (2026-07-05). The version-pin + kind parsing + units boundary +
the round-trip contract are implemented; per-entity field mapping is filled for
the load-bearing entities (Well, Wellbore, WellboreTrajectory) and extends by the
same pattern. See ``docs/dev/HIERARCHY_FRAMEWORK.md`` for the schema map and
``https://community.opengroup.org/osdu/data/data-definitions`` for the schemas.
"""
